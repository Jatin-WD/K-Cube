    ...
    return (
      <main className="min-h-screen bg-[#070708] px-5 py-16 text-white lg:px-10">
        <section className="mx-auto max-w-2xl rounded-xl border border-white/10 bg-[#111113] p-8 text-center">
          ...
          <h1 className="mt-5 text-3xl font-black">{t.signInProfile}</h1>
          <p className="mt-3 text-sm leading-7 text-[#aab5c6]">{t.signInProfileDesc}</p>
          <Link href="/signin?returnTo=/profile" className="mt-6 inline-flex rounded-lg bg-[#ffc400] px-5 py-3 text-sm font-black text-[#090909]">{t.signIn}</Link>
        </section>
      </main>
    );
  }
  ...
  return (
    <main className="min-h-screen bg-[#070708] px-4 py-8 text-white sm:px-5 lg:px-10 lg:py-12">
      <div className="mx-auto max-w-[1180px]">
        <Link href="/dashboard" className="inline-flex items-center gap-2 text-sm font-bold text-[#aab5c6] transition hover:text-[#ffc400]"><ArrowLeft className="h-4 w-4" /> {t.backDashboard}</Link>
          ...
          {loading ? <div className="p-8 text-sm text-[#aab5c6]">Loading your profile...</div> : (
            <div className="grid gap-6 p-6 sm:p-8 lg:grid-cols-[1fr_0.7fr]">
              <form onSubmit={saveProfile} className="space-y-5">
                ...
                <button disabled={saving} className="inline-flex items-center gap-2 rounded-lg bg-[#ffc400] px-5 py-3 text-sm font-black text-[#090909] disabled:opacity-60"><Save className="h-4 w-4" /> {saving ? t.saving : t.saveChanges}</button>
              </form>

              <aside className="space-y-4">
                <h2 className="text-2xl font-black">{t.account}</h2>
