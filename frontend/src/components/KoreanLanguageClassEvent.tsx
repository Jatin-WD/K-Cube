...
const formatTime = (value: string, _timezone: string, language: Language) => {
...
const statusLabel = (status: string, language: Language) => status === 'full'
  ? language === 'ko' ? '\uB9C8\uAC10' : language === 'hi' ? '\u0938\u0940\u091F\u0947\u0902 \u092D\u0930 \u0917\u0908\u0902' : 'Full'
  : language === 'ko' ? '\uC885\uB8CC\uB428' : language === 'hi' ? '\u092A\u0942\u0930\u093E \u0939\u094B \u091A\u0941\u0915\u093E' : 'Completed';
  ...
  return <main className="min-h-screen bg-[#eef4f8] text-[#102a43]">
    <section className="bg-[linear-gradient(135deg,#062b63_0%,#0b4eae_58%,#123b78_100%)] px-4 py-12 text-white sm:px-8 sm:py-16 lg:px-10">
      <div className="mx-auto max-w-[1200px]">
        ...
        <div className="mt-7 flex flex-wrap gap-3 text-sm font-bold text-[#e0ecff]"><span className="inline-flex items-center gap-2 rounded-full bg-white/10 px-4 py-2"><CalendarDays className="h-4 w-4" />{t.everyTuesday}</span><span className="inline-flex items-center gap-2 rounded-full bg-white/10 px-4 py-2"><Clock3 className="h-4 w-4" />{firstEvent ? `${formatTime(firstEvent.starts_at, firstEvent.timezone, language)}–${formatTime(firstEvent.ends_at, firstEvent.timezone, language)} ${firstEvent.timezone}` : '3:00–4:00 PM Asia/Kolkata'}</span><span className="inline-flex items-center gap-2 rounded-full bg-white/10 px-4 py-2"><MapPin className="h-4 w-4" />{firstEvent?.location_name || t.locationSoon}</span></div>
      </div>
    </section>
    <section className="px-4 py-8 sm:px-8 sm:py-12 lg:px-10">
      <div className="mx-auto grid max-w-[1200px] gap-6 lg:grid-cols-[1.15fr_0.85fr]">
        <article className="rounded-[28px] border border-[#d8e1ee] bg-white p-6 shadow-[0_18px_50px_rgba(15,55,95,0.07)] sm:p-8">
          ...
          <p className="mt-3 rounded-xl border border-[#f1d98a] bg-[#fff9e5] p-3 text-sm font-bold text-[#806000]">{t.rewardShort}</p>
          <div className="mt-6 grid gap-3 sm:grid-cols-2">{loading ? <p className="text-sm text-[#64748b]">{t.loading}</p> : events.map((event, index) => <div key={event.id} className="rounded-2xl border border-[#d8e1ee] bg-[#f8fbff] p-4"><p className="text-xs font-black uppercase tracking-[0.16em] text-[#2457d6]">{t.week} {index + 1}</p><p className="mt-2 text-lg font-black">{formatDate(event.starts_at, language)}</p><p className="mt-1 text-sm text-[#64748b]">{formatTime(event.starts_at, event.timezone, language)}–{formatTime(event.ends_at, event.timezone, language)} · {event.timezone}</p><p className="mt-2 text-xs font-bold text-[#087f52]">+{event.points_reward || 100} {t.verified}</p></div>)}</div>
          <div className="mt-6 rounded-2xl border border-[#b9d9d0] bg-[#effaf6] p-5"><p className="flex items-center gap-2 text-sm font-black text-[#087f52]"><MapPin className="h-4 w-4" />{firstEvent?.location_name || t.locationSoon}</p><p className="mt-2 text-sm leading-6 text-[#486581]">{firstEvent?.location_address || t.venueSoon}</p></div>
        </article>
