...
const participantActions = [
  ...
  ['02', 'Final Selection Point Award', '1,000 points are awarded after passing the final selection.'],
...
export default function AnnouncementPage() {
  ...
  return (
    <main className="min-h-screen bg-[#eef4f8] text-[#102a43]">
      ...
      <section className="px-3 pb-6 sm:px-4 sm:pb-8 lg:px-10"><div className="mx-auto max-w-[1240px] rounded-[24px] border border-[#cbd9ea] bg-white p-6 sm:p-8 lg:p-9"><SectionTitle eyebrow="For existing participants" title="What Should I Do Now?" description="The application window is closed, but current participants should keep their account active and follow the operational updates below." /><div className="mt-6 grid gap-4 lg:grid-cols-3">{participantActions.map(([number, title, text], index) => <article key={title} className={`rounded-[18px] border p-5 ${index === 1 ? 'border-[#e5c56b] bg-[#fffdf5]' : 'border-[#d8e1ee] bg-[#f8fbff]'}`}><div className="flex items-center justify-between"><span className="flex h-8 w-8 items-center justify-center rounded-full bg-[#e8f0ff] text-xs font-black text-[#2457d6]">{number}</span>{index === 1 && <span className="text-xs font-black text-[#b77900]">1,000 points awarded after passing final selection</span>}</div><h3 className="mt-5 text-lg font-black text-[#102a43]">{title}</h3><p className="mt-3 text-sm leading-7 text-[#526f8f]">{text}</p>{index === 1 && <p className="mt-3 text-xs leading-5 text-[#526f8f]">1,000 points are awarded after passing the final selection. No daily target is required for this award.</p>}{index === 2 && <Link href="/rewards" className="mt-4 inline-flex items-center gap-2 text-sm font-black text-[#2457d6]">Explore ways to earn points<ArrowRight className="h-4 w-4" /></Link>}</article>)}</div></div></section>
