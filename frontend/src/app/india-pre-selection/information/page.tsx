...
const bottomLinks = [
  ...
  },
  {
    ...
    description: 'Move to the application page once you are ready to apply.',
  },
];

export default function InformationPage() {
  return (
    <main className="min-h-screen bg-[#ece8dc] text-[#111827]">
      <IndiaPreSelectionInformationHero />

      <section className="px-3 pb-8 sm:px-4 sm:pb-10 lg:px-10">
        <div className="mx-auto max-w-[1760px] rounded-[30px] border border-[#d5d9d9] bg-white px-5 py-5 shadow-sm sm:px-6 sm:py-6 lg:px-8 lg:py-7">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
            ...
            <p className="max-w-3xl text-sm leading-7 text-[#565959]">
              This section stays broad on purpose so visitors understand the festival before they move to notices or application details.
            </p>
          </div>
          ...
          <div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
            {[
              ...
              {
                title: 'Next move',
                text: 'Read this page first, then open Announcement for updates and Apply for submission.',
              },
            ].map((item) => (
          ...
          <div className="mt-5 rounded-[28px] border border-[#d5d9d9] bg-[#111827] p-4 text-white sm:p-5">
            <p className="text-xs font-black uppercase tracking-[0.18em] text-[#f3a847]">Process overview</p>
            <div className="mt-3 grid gap-3 md:grid-cols-3">
              {processSteps.map((step) => (
                <div key={step} className="flex items-center rounded-[22px] border border-white/10 bg-white/[0.03] px-4 py-3 text-sm leading-6 text-[#e5e7eb]">
                  {step}
                </div>
              ))}
            </div>
      ...
      <section className="px-3 pb-10 sm:px-4 sm:pb-12 lg:px-10">
        <div className="mx-auto max-w-[1760px] rounded-[30px] border border-[#d5d9d9] bg-white px-5 py-5 shadow-sm sm:px-6 sm:py-6 lg:px-8 lg:py-7">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
            ...
            <p className="max-w-3xl text-sm leading-7 text-[#565959]">
              Start with Information, check Announcement for live updates, and move to Apply only when you are ready to submit.
            </p>
          </div>
          ...
          <div className="mt-5 grid gap-3 overflow-hidden rounded-[24px] border border-[#d5d9d9] bg-[#f7fafa] lg:grid-cols-3 lg:gap-0">
            {bottomLinks.map((item, index) => (
              <Link
                key={item.label}
                href={item.href}
                className="group border border-[#d5d9d9] bg-white p-4 transition hover:bg-[#fff8df] lg:border-y-0 lg:border-l-0 lg:border-r lg:last:border-r-0 lg:bg-transparent lg:p-5"
              >
